...
import { FiSearch, FiMapPin, FiBookmark, FiThumbsDown, FiShare2, FiExternalLink, FiCheckCircle } from "react-icons/fi";
...
function CandidateDashboard() {
  ...
  const handleHideCurrentJob = async (jobId) => {
    if (!jobId) return;
    try {
      await hideJob(jobId);
      toast.success("Job hidden from your feed");
      setJobs((prev) => prev.filter((j) => j._id !== jobId));
      setRecommendedJobsList((prev) => prev.filter((j) => j._id !== jobId));
  ...
  const handleShareJob = (job) => {
    if (!job) return;
    const shareUrl = `${window.location.origin}/candidate-dashboard?jobId=${job._id}`;
    if (navigator.clipboard) {
      navigator.clipboard.writeText(shareUrl);
      toast.success("Job link copied to clipboard!");
    } else {
      toast.success("Job link ready to share!");
    }
  };
                ...
                <div className="space-y-3">
                  {visibleJobs.map((job) => {
                    ...
                    return (
                      <div
                        key={job._id}
                        className={`bg-white border rounded-2xl p-4 sm:p-5 cursor-pointer transition-all ${
                          isSelected
                            ? "border-brand-600 ring-2 ring-brand-500/20 bg-blue-50/20 shadow-md"
                      ...
                      >
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <div className="min-w-0 flex-1">
                            <h4 className="text-base font-extrabold text-slate-900 leading-snug tracking-tight mb-1 truncate group-hover:text-brand-600">
                              {job.title}
                            </h4>
                          ...
                          <div className="flex items-center gap-1 shrink-0 -mr-1 -mt-1">
                            <button
                              type="button"
                              className="p-1.5 text-slate-400 hover:text-rose-500 transition-colors border-0 bg-transparent cursor-pointer rounded-lg hover:bg-slate-50"
                              aria-label="Hide job"
                              title="Not interested"
                              onClick={(e) => {
                                e.stopPropagation();
                                handleHideCurrentJob(job._id);
                              }}
                            >
                            ...
                            <button
                              type="button"
                              className="p-1.5 text-slate-400 hover:text-brand-600 transition-colors border-0 bg-transparent cursor-pointer rounded-lg hover:bg-slate-50"
                              aria-label="Save job"
                              title="Save job"
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            >
                              {user?.savedJobs?.some((savedJobId) => savedJobId?.toString() === job?._id) ? (
                                <FaBookmark size={15} className="text-brand-600" />
                              ) : (
                                <FiBookmark size={15} />
                              )}
                            </button>
                        ...
                        {/* Salary Highlight Pill (Indeed Style) */}
                        {job.salary || job.salaryMin ? (
                          <div className="mb-2.5">
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-bold">
                              <span className="text-emerald-600 font-black">✓</span>
                              <span>{formatSalary(job.salary, job.salaryMin, job.salaryMax, job.salaryCurrency)}</span>
                            </span>
                          </div>
                        ) : null}
            ...
            {selectedJob ? (
              <div className="flex flex-col h-full overflow-hidden">
                  ...
                  <div className="pt-2">
                    {selectedJob.expiresAt && new Date(selectedJob.expiresAt) < new Date() ? (
                      ...
                      <div className="flex flex-wrap items-center gap-2.5">
                        {!externalApplyActive ? (
                          <button 
                            className="px-6 py-2.5 bg-brand-600 hover:bg-brand-700 active:scale-95 text-white font-extrabold text-sm rounded-xl cursor-pointer transition-all shadow-sm border-0 inline-flex items-center gap-2"
                            onClick={handleApplyNowClick}
                          >
                            <span>{selectedJob.isExternal ? "Apply on company site" : "Apply now"}</span>
                            {selectedJob.isExternal && <FiExternalLink size={15} />}
                          </button>
                        ) : (
                          <>
                            <button 
                              className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-extrabold text-sm rounded-xl cursor-pointer transition-all shadow-sm border-0"
                              onClick={handleManualTrack}
                            >
                              Mark as Applied
                            </button>
                            <button 
                              className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-700 font-extrabold text-sm rounded-xl cursor-pointer transition-all border border-slate-300"
                              onClick={handleApplyNowClick}
                            >
                              Open Again
                            </button>
                        ...
                        >
                          {user?.savedJobs?.some(
                            ...
                            <FaBookmark size={16} className="text-brand-600" />
                          ) : (
                            <FiBookmark size={17} />
                          )}
                        </button>
                        ...
                        <button
                          type="button"
                          className="w-10 h-10 rounded-xl flex items-center justify-center border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all cursor-pointer"
                          onClick={() => handleHideCurrentJob(selectedJob._id)}
                          title="Not interested / Hide job"
                        >
                ...
                <div className="p-6 overflow-y-auto flex-1 space-y-6 custom-scrollbar">
                  {/* Indeed-Style Job Details Summary Section */}
                  <div className="p-5 bg-slate-50/80 border border-slate-200/80 rounded-2xl space-y-4">
                    <div>
                      <h4 className="text-base font-extrabold text-slate-900 m-0">Job details</h4>
                      <p className="text-xs text-slate-500 m-0 mt-0.5">Here's how the job details align with your profile</p>
                    </div>

                    <div className="space-y-3.5 pt-1">
                      {/* Pay Highlight */}
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-lg bg-emerald-100/80 text-emerald-700 flex items-center justify-center shrink-0 mt-0.5 font-black text-sm">
                          ₹
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-700 m-0 mb-1">Pay</p>
                          <span className="inline-flex items-center gap-1 px-3 py-1 bg-emerald-100/70 text-emerald-800 text-xs font-bold rounded-lg border border-emerald-200">
                            <span className="text-emerald-600 font-black">✓</span>
                            <span>{formatSalary(selectedJob.salary, selectedJob.salaryMin, selectedJob.salaryMax, selectedJob.salaryCurrency)}</span>
                          </span>
                        </div>
                      </div>
                      ...
                      {/* Job Type Highlight */}
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-lg bg-blue-100/80 text-brand-700 flex items-center justify-center shrink-0 mt-0.5 font-black text-sm">
                          💼
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-700 m-0 mb-1">Job type</p>
                          <span className="inline-block px-3 py-1 bg-slate-200/70 text-slate-800 text-xs font-bold rounded-lg border border-slate-300">
                            {selectedJob.employmentType || selectedJob.role || "Full-time"}
                          </span>
                        </div>
                      </div>

                      {/* Qualifications & Profile Alignment */}
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-lg bg-purple-100/80 text-purple-700 flex items-center justify-center shrink-0 mt-0.5 font-black text-sm">
                          🎯
                        </div>
                        <div className="flex-1">
                          <p className="text-xs font-bold text-slate-700 m-0 mb-1.5">Qualifications</p>
                          <div className="space-y-2">

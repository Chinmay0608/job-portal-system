...
async function updateJobs() {
  ...
  // 1. Update AI Engineer job directly to bypass validation if missing fields
  await Job.updateOne(
    { title: { $regex: /AI Engineer/i } },
    { 
      $set: { 
        createdAt: new Date(),
        description: `Are you passionate about building intelligent systems and solving complex problems with machine learning? We are looking for an innovative AI Engineer to join our core AI and Research team.
...
• Experience with cloud platforms (AWS, GCP, Azure) and ML deployment tools (Docker, Kubernetes, MLflow) is a strong plus.` 
  ...
  // 2. Fetch ~50 other external jobs and make them "new" using updateMany
  const externalJobs = await Job.find({ isExternal: true }).limit(50);
  let count = 0;
  for (let job of externalJobs) {
    const offset = Math.floor(Math.random() * (2 * 24 * 60 * 60 * 1000));
    await Job.updateOne({ _id: job._id }, { $set: { createdAt: new Date(Date.now() - offset) } });
    count++;
  }
  console.log(`Updated \${count} external jobs to be "new"`);
